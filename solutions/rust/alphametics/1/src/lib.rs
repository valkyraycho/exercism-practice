...
fn backtrack(
    ...
    for digit in 0..=9u8 {
        if used[digit as usize] || (digit == 0 && leading.contains(&letter)) {
            continue;
        }
        mapping.insert(letter, digit);
        used[digit as usize] = true;

        if let Some(solution) = backtrack(remaining, mapping, used, leading, left, right) {
