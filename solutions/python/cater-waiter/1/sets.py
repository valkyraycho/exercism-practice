...
def categorize_dish(dish_name, dish_ingredients):
    ...
    for i, category in enumerate([VEGAN, VEGETARIAN, KETO, PALEO, OMNIVORE, ALCOHOLS, SPECIAL_INGREDIENTS]):
        if set(dish_ingredients) <= category:
            return f"{dish_name}: {categories[i]}"
...
def singleton_ingredients(dishes, intersection):
    ...
    result = set()
    for dish in dishes:
        result |= dish.symmetric_difference(intersection) - intersection
    return result